 ...
 * 
 * Detailed analytics view for buyer activity
 * Shows comprehensive statistics, trends, and insights
 */
...
export default function BuyerAnalyticsPage() {
  ...
  const fetchStats = async () => {
    try {
      ...
      if (apiCategoryData.length > 0) {
        ...
        setCategoryData([{ name: 'No data yet', value: 1 }]);
      }

      setLoading(false);
    } catch (error) {
      console.error('[Buyer Analytics] Error fetching statistics:', error);
  ...
  const activityRadarData = {
    labels: ['Searches', 'Views', 'Favorites', 'Inquiries', 'Conversations'],
    datasets: [
      {
        label: 'Activity Level',
        data: [
          stats.totalSearches,
          stats.businessesViewed,
    ...
    <Box sx={{ 
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body2" color="text.secondary">
          Comprehensive insights into your buying activity and engagement
        </Typography>
      </Box>
